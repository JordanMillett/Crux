...
public static class Input
{
    ...
    public static bool IsActionHeld(string action)
    {
        ...
        if (keybindings.ContainsKey(action))
        {
            return Crux.Engine.IsKeyDown(keybindings[action].key);
        }else
        {
    ...
    public static bool IsActionPressed(string action)
    {
        ...
        if (keybindings.ContainsKey(action))
        {
            return Crux.Engine.IsKeyPressed(keybindings[action].key);
        }else
        {
            Logger.LogWarning($"Action '{action}' is unbound.");
            return false;
    ...
    public static bool IsActionReleased(string action)
    {
        ...
        if (keybindings.ContainsKey(action))
        {
            return Crux.Engine.IsKeyReleased(keybindings[action].key);
        }else
        {
            Logger.LogWarning($"Action '{action}' is unbound.");
            return false;
