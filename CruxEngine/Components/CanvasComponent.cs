...
{          
    ...
    {
        float screenWidth = Crux.Engine.Resolution.X;
        float screenHeight = Crux.Engine.Resolution.Y;
        float scaleX = width / screenWidth;
        float scaleY = height / screenHeight;

        Matrix4 scale = Matrix4.CreateScale(scaleX, scaleY, 1f);

        float ndcX = (xpos / Crux.Engine.Resolution.X) * 2f - 1f;
        float ndcY = -((ypos / Crux.Engine.Resolution.Y) * 2f - 1f);
